...
import { Search, BookOpen, Activity, LayoutDashboard, Cpu } from 'lucide-react';
...
interface HeaderProps {
  ...
  onOpenReadme: () => void;
  categories: string[];
  viewMode: 'dashboard' | 'model';
  setViewMode: (mode: 'dashboard' | 'model') => void;
...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="border-b border-slate-800 bg-slate-950/90 backdrop-blur-md sticky top-0 z-30 px-6 py-3 flex flex-col md:flex-row items-center justify-between gap-4">
      {/* Brand & View Mode Navigation */}
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="font-bold text-lg tracking-tight text-white font-mono">
                QUANT<span className="text-cyan-400">LABS</span>
              </h1>
      ...
      {/* Quick Ticker Mock (Large Screens) */}
      <div className="hidden lg:flex items-center gap-4 text-xs font-mono pl-4 border-l border-slate-800/80">
      ...
      {/* Search Input */}
      <div className="flex flex-1 items-center justify-center max-w-md w-full gap-3">
        <div className="relative w-full">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
          <input
            type="text"
            placeholder="Search quantitative models (e.g. Black-Scholes, VaR)..."
            value={searchQuery}
